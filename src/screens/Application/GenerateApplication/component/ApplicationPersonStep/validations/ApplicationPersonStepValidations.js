...
import {EMAIL_ADDRESS_REGEX, SPECIAL_CHARACTER_REGEX} from "../../../../../../constants/RegexConstants";
    ...
    const partners = data?.map((item, index) => {
        ...
        if (!isDisabled) {
            if (type === 'company') {
                ...
                if (item?.abn && item?.abn.trim().length !== 11 && isNaN(item?.abn)) {
                    validated = false;
                    errors.abn = 'Please enter valid ABN number before continue';
                }
                if (item?.acn && item.acn?.trim().length !== 9 && isNaN(item?.acn)) {
                    validated = false;
                    errors.acn = 'Please enter valid ACN number before continue';
                }
                if (!item?.entityName || item?.entityName.value.length <= 0) {
                    validated = false;
            ...
            if (type === 'individual') {
                ...
                if (item?.firstName && item.firstName && SPECIAL_CHARACTER_REGEX.test(item.firstName?.toString())) {
                    validated = false;
                ...
                if (item?.lastName && item?.lastName && SPECIAL_CHARACTER_REGEX.test(item?.lastName?.toString())) {
                    validated = false;
                ...
                if(isNaN(item?.mobileNumber)){
                    validated = false;
                    errors.mobileNumber = 'Mobile number should be number';
                }
                if(isNaN(item?.driverLicenceNumber)){
                    validated = false;
