...
export const saveEmployeeColumnList = ({employeeColumnList = {}, isReset = false}) => {
    return async dispatch => {
        ...
        try {
            ...
            if (!isReset) {
                const defaultFields = employeeColumnList.defaultFields.filter(field => field.isChecked).map(field => field.name);
                const customFields = employeeColumnList.customFields.filter(field => field.isChecked).map(field => field.name);
                data = {
