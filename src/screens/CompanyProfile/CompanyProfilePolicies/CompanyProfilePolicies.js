...
import {
    ...
    getCompanyProfilePolicyList, saveCompanyProfilePolicyColumnList
...
const CompanyProfilePolicies = () => {
    ...
    const policyList = useSelector(({companyProfile}) => companyProfile?.clientPolicyData?.clientPolicyList ?? {});
    const companyProfilePolicyColumnList = useSelector(({companyProfile}) => companyProfile?.clientPolicyData?.clientPolicyColumnList ?? {})
    const companyProfilePolicyDefaultColumnList = useSelector(({companyProfile}) => companyProfile?.clientPolicyData?.clientPolicyDefaultColumnList ?? {})
    const {docs, page, limit, pages, total, headers, isLoading} = useMemo(() => policyList, [policyList]);
    const [pageLimit, setPageLimit] = useState('');
    const [customFieldModal, setCustomFieldModal] = useState(false);
    const {defaultFields, customFields} = useMemo(() => companyProfilePolicyColumnList ?? {
        defaultFields: [],
        customFields: []
    ...
    const onClickSaveColumnSelection = useCallback(async () => {
        try {
            const isBothEqual = _.isEqual(companyProfilePolicyColumnList, companyProfilePolicyDefaultColumnList);
            if (!isBothEqual) {
    ...
    }, [getPolicyList, toggleCustomField, companyProfilePolicyColumnList, companyProfilePolicyDefaultColumnList])
            ...
            <>
                <div className="common-white-container">
                    <div className="page-header mt-15 mb-15">
                        ...
                        {!isLoading && docs && (<div className="buttons-row">
                            <BigInput ref={searchInputRef} prefix="search" prefixClass="font-placeholder" placeholder="Search here"
