...
const CreditLimitsApplicationTab = () => {
  ...
  const creditLimitsApplicationColumnList = useSelector(
    ({ creditLimits }) => creditLimits?.application?.applicationColumnList ?? {}
  );
  const creditLimitsApplicationDefaultColumnList = useSelector(
    ({ creditLimits }) => creditLimits?.application?.applicationDefaultColumnList ?? {}
  );
  const { total, headers, pages, docs, page, limit, isLoading } = useMemo(
    () => creditLimitsApplicationList,
    [creditLimitsApplicationList]
  );
  ...
  const onClickSaveColumnSelection = useCallback(async () => {
    try {
      const isBothEqual = _.isEqual(
        creditLimitsApplicationColumnList,
        creditLimitsApplicationDefaultColumnList
      );
      if (!isBothEqual) {
        await dispatch(
          onSaveCreditLimitsApplicationColumnList({ creditLimitsApplicationColumnList })
        );
        await getApplicationList();
        toggleCustomField();
      } else {
